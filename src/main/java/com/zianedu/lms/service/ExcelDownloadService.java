...
public class ExcelDownloadService extends AbstractExcelView {
    ...
    protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest req, HttpServletResponse res) throws Exception {
        ...
        switch (excelContent) {
            ...
            break;
            case "cancelOrderList" : cancelOrderListExcelDownload((List<OrderExcelDownDTO>)model.get(DATA_LIST), sheet);
                break;
            default:
            break;
        }
    ...
    private void cancelOrderListExcelDownload(List<OrderExcelDownDTO>dataList, HSSFSheet sheet) {
        if (dataList.size() > 0) {
            int i=0;
            for (OrderExcelDownDTO dto : dataList) {
                String goodsTypeName = "";
                ...
                row.createCell(8).setCellValue(dto.getStartDate());
                row.createCell(9).setCellValue(goodsTypeName);
                row.createCell(10).setCellValue(orderName);
                row.createCell(11).setCellValue(dto.getIsOffline() == 0 ? "X": "O");
                row.createCell(12).setCellValue(NumberFormat.getInstance().format(dto.getGoodsPrice()));
                row.createCell(13).setCellValue(NumberFormat.getInstance().format(dto.getSellPrice()));
                row.createCell(14).setCellValue(NumberFormat.getInstance().format(dto.getPricePay()));
                row.createCell(15).setCellValue(OrderPayType.getOrderPayTypeStr(dto.getPayType()));
                row.createCell(16).setCellValue(dto.getBank());
                row.createCell(17).setCellValue(dto.getBankAccount());
                row.createCell(18).setCellValue(dto.getDepositUser());
                row.createCell(19).setCellValue(dto.getDepositDate());
                row.createCell(20).setCellValue(OrderPayStatusType.getOrderPayStatusStr(dto.getPayStatus()));
                row.createCell(21).setCellValue(dto.getIsCancelRequest() == 0 ? "X" : "O");
                row.createCell(22).setCellValue(dto.getPayStatus()==0? "":dto.getPayDate());
                row.createCell(23).setCellValue(dto.getCancelDate());
                row.createCell(24).setCellValue(dto.getIsMobile() == 0 ? "X" : "O");
                row.createCell(25).setCellValue(deliveryPrice);
                row.createCell(26).setCellValue(DeliveryStatusType.getDeliveryStatusName(Integer.parseInt(Util.isNullValue(dto.getDeliverStatus(), "0"))));
                row.createCell(27).setCellValue(dto.getDeliveryZipcode());
                row.createCell(28).setCellValue(dto.getDeliveryAddressRoad());
                row.createCell(29).setCellValue(dto.getDeliveryAddress());
                row.createCell(30).setCellValue(CashReceiptType.getCashReceiptTypeStr(dto.getCashReceiptType()));
                row.createCell(31).setCellValue(dto.getCashReceiptNumber());
